...
author: "Shipra Ahuja, Ben Arancibia, Xingjia Wu"
...
There are two basic bets in craps, pass and do no??t pass. In the pass bet, you wager that the shooter (the person throwing the dice) will win; in the do no??t pass bet, you wager that the shooter will lose. We will play by the rule that on an initial roll of 12 ("boxcars"), both pass and do no??t pass bets are losers. Both are even-money bets.

Conduct of the game:
Roll a 7 or 11 on the first roll: Shooter wins (pass bets win and do not pass bets lose). Roll a 12 on the first roll: Shooter loses (boxcars; pass and do not pass bets lose).
Roll a 2 or 3 on the first roll: Shooter loses (pass bets lose, do no?t pass bets win).
...
The shooter continues to roll the dice until the point or a 7 appears. Pass bettors win if the shooter rolls the point again before rolling a 7. Do not pass bettors win if the shooter rolls a 7 before rolling the point again.

Write an algorithm and code it in the computer language of your choice. Run the simulation to estimate the probability of winning a pass bet and the probability of winning a do no?t pass bet. Which is the better bet? As the number of trials increases, to what do the probabilities converge?
...
__Chapter 9 Section 3 Project 4__
...
![](//users/bcarancibia/CUNY_IS_609/courseproject/Code/tree.jpg)
...
__Chapter 11 Section 5 Project 5__
...
dN/dt = 0.25N(10 - N),N(0)=2   

(a)where N is measured in 100's. Analyze the behavior of this differential equation as
follows.
...
![Image for Phase Line First Derivative](//users/bcarancibia/CUNY_IS_609/courseproject/Code/Project_FirstDerivative_PhaseLine.jpg)
...
![Image for Phase Line Second Derivative](//users/bcarancibia/CUNY_IS_609/courseproject/Code/Project_SecondDerivative_PhaseLine.jpg)

**Solution Curve based on Phase Line**

![Image for Solution Curve](//users/bcarancibia/CUNY_IS_609/courseproject/Code/SolutionCurves.jpg)
